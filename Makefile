...
#copy static files to the installdir
	cp *.ico *.html *.php robots.txt $(INSTALLDIR)/www

# Copy js and css files to the installdir
...
ifeq ($(TOMATO_EXPERIMENTAL),1)
	...
	cd $(INSTALLDIR)/www && \
	for F in $(wildcard *.asp); do \
		sed -e "s,<div class='version'>Version <% version(); %></div>,<div class='version'>Version <% version(); %> by Ratatosk</div>," $$F > $$F.tmp; \
		mv $$F.tmp $$F; \
	done
...
# Statements are moved up in this installation file by Ratatosk, to ensure the security and cleanup 
# functions affect these files too 
...
# Yuicompressor dowloaded from: https://github.com/yui/yuicompressor/releases
	cp yuicompressor/yuicompressor-2.4.8.jar $(INSTALLDIR)/www
...
# secure the files in the installdir.
# Command is changed by Ratatosk, to change the only file ACLs (and preserve the existing folder ACLs)
...
# Ratatosk: Obsolete for js files because of usage of yuicompressor, but not removed yet.
